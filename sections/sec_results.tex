...
%The results obtained from the systematic literature review are plotted in different graphs and they are given below in a way to answer the research questions.

%From the analysis using the Systematic literature approach, we can see from Figure~\ref{fig:total-papers-per-year}, 'Publication year vs articles' that the articles are been published increasing high from the year 2017. This gives us the significance of the study related to microservices and technologies that are growing day by day in developing microservices. This makes us easier to understand that during the years after the introduction of microservices after 2017 a two years later, we can gather good enough challenges from then till date. This is addressed in the R4 of the exclusion criteria. One should be aware of the challenges which are faced in adopting microservices as this is also our motivation to provide an overview of the challenges faced and to overcome them while developing microservices. 
...
%To provide an overview of the number and type of publications analyzed we provided the literature statistics in Figure~\ref{fig:selected-papers-grey-1}, \ref{fig:selected-papers-grey-2} and Figure~\ref{fig:selected-papers-slr-1}, \ref{fig:selected-papers-slr-2}. 
...
\begin{figure*}[t]
	\centering
	\includegraphics[width=\linewidth]{images/Challenges_updated.png}
...
%Organizations are increasingly shifting to a microservices architecture to support faster time to market. However, load balancing challenges often result as we resource population changes, and as we infrastructure grows to meet demand. If the load is not shared evenly across nodes, application performance suffers and end-user experience can be impacted. Determining how we load is balanced across all the nodes for each service in we environment and dealing with dynamic changes. One of the challenges remains how to quickly roll out, troubleshoot, and manage these microservices. Manually allocating resources for applications and reconfiguring the load balancer to incorporate new services is inefficient and error-prone. It becomes downright problematic at scale~\cite{Ville2019, Branko2018}.
